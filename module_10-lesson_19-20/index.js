...
function handleArticles({articles, totalResults}) {
  totalPages = calculatePagination(totalResults)
...
searchBtnRef.addEventListener('click', e => getNews(searchField.value).then(handleArticles));
...
paginationContainer.addEventListener('click', e => {
  ...
  getNews(searchField.value).then(handleArticles)
})
 ...
 */
const createArticlesElements = articles => articles.map(({title, description, url, urlToImage}) => {
 ...
 */
const calculatePagination = totalResults => Math.floor((totalResults > 100 ? 100 : totalResults) / PageSize)
